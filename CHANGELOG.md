...
**判据修正／输出新增版（patch）**：**对外能力不变**（子命令总数仍 **15**、**无新参数、无新子命令**）；`check` **新增第 ⑧ 维**，但它**只提示、不进退出码**。

- ⭐ **`check` 新增第 ⑧ 维「加载链完整性」（只提示）**：判据对齐**框架的取键方式** —— `children` 须**存在＋数组＋非空**、`children[0].configs` 与 `roles` 须**存在＋对象**（框架这几处是"**缺失即抛、类型不符即抛**"）⇒ 这类文件**能被 ④ 解析、页面却在加载期打不开**。定位是"**防手写 / 防半成品**"（`new` 的产物一定完整）。
- ⭐ **报 `[warn]` 但退出码仍 0**：⑧ 违规时只打 `[warn]`，**`check` 的退出码仍是 0**、**不会让 CI / pre-commit 变红**（它**不阻塞**）。⑧ **恒留一行**、三形态定死：无异常 ⇒ `[note] ⑧ 加载链完整性：无异常`；有违规 ⇒ `[warn] ⑧ 加载链完整性：…`（逐条含**键名与期望类型**）；④ 解析失败 ⇒ `[note] ⑧ 加载链完整性：因解析失败跳过`（**这行的 rc=1 是 ④ 给的、不是 ⑧**）。
- **`patch` / `edit` / `expand` 写盘后的自动校验不纳入 ⑧**（与 ⑥⑦ 同口径：写盘后只判 ①–⑥）⇒ 写盘不会因 ⑧ 多报、也不会把退出码顶红；要看 ⑧ 请单独跑 `check`。

### 升级须知（新增 / 变严）

**新增 / 放松**

- **`check` 多一项非阻塞的体检结论**（第 ⑧ 维「加载链完整性」）：正常文件照旧打一行 `[note] ⑧ 加载链完整性：无异常`；有问题则多打 `[warn] ⑧ …` 行 —— **退出码不变**（①–⑤ 判定面未动）。
...
**判据修正版（patch）**：**对外能力不变**（子命令总数仍 **15**、无新参数、无新子命令）。本版**只放宽读入** ＋ 修正一处输出，**没有变严项**。

- ⭐ **读入面放宽（本版对外最大的变化）**：**工具现在接受带尾随逗号的 `.xwl`**（例 `{"a":1,}`）—— 对齐框架加载器 `org.json` 的宽容面。因 `parse_xwl` 是**所有读盘路径的唯一入口**，故 **`check` / `patch` / `expand` / `edit` / `dump` / `params` / `sqlrefs` 同源放宽**。⚠️ **两点说清**：① **只放宽「读入」** —— **不表示**「尾随逗号是合法 JSON」（按 JSON 规范它**仍是**格式问题）；② **写盘会把它归一掉** —— 工具按设计器自己的算法重建文件时**不会保留**该逗号。
- ⭐ **一处输出修正（`diffguard`）**：精确命中在**工作区该行带尾随空白**时，旧版会报出**行号 0**（`被合并成工作区第 0 行`）；本版改报**真实物理行号**。判定不变，只修报出的行号。

### 升级须知（放松 / 变严）

**放松（此前会 FAIL 的输入，现在通过）**

- **带尾随逗号的 `.xwl`**：`check` 的 ④（加载器等价解析）不再判 `[FAIL]`；`patch` / `expand` / `edit` / `dump` / `params` / `sqlrefs` 也能读入。

**变严（既有文件可能因此新报问题）**

- **无** —— 本版没有变严项（只有放松 ＋ 一处输出修正）。

## [1.4.0] - 2026-09-23

**新增对外能力版（minor）**：`params` 新增**两个参数**（`--upstream` **默认关**；`--strict` **本版与默认同效**，为下一版的默认值翻转预留）；**子命令总数仍 15、无新子命令**。对外能力有**新增（新参数）** ⇒ 按本仓尺子发 **minor**。

### 升级须知（新增 / 口径变化 / 迁移预告）

**新增 / 变强**

- **`params --upstream`（默认关）**：核对「**谁在调用本页**」—— 在 `--module-root` 内**单根全扫**一遍，末尾新增**独立分组** `[外部可传入]`（上游调用方与传入键），再给一行**载入侧汇总**（**节点级三分类**「同根命中 / 本根未找到 / 非 `m?xwl` 的 url」＋**出现级**「动态写法 / 非 `m?xwl` 字面量」，**两种量纲分开标**；节点级三数**之和 = 本页 `store.url` 数**）。它**只认 `params: {名: 值}` 字面量键**（`out: app.<容器>` 与运行时拼接的 url **只计数**）；**不并入 `provided`、不改退出码**。

**口径变化 / 输出变化（既有用户会看到差异）**

- **`params` 缺来源时新增一行留痕**：有 miss 且未给 `--strict` 时，`params` 输出末尾多打一行 `[note]` 预告「默认仍阻塞（rc=1）；下一版起默认不再阻塞，届时要阻塞请加 `--strict`」 ⇒ **有 miss 的页面输出会比旧版多一行**；**无 miss 的页面输出逐字不变**。⚠️ **`--upstream` 默认关时不出现新分组与汇总行**（零成本）；上面那行 `[note]` 是**唯一的例外** —— 它由「缺来源」触发、由 `--strict` 抑制，**与 `--upstream` 无关**。
- **`folders` 结论行改报 `index` 悬空项**：「未登记」移出结论行、只进明细；`folder.json 损坏` 仍在结论行。

**迁移预告（下一版）**

- 本版**没有**破坏性变更。**下一版（`1.5.0`）**将把 `params` 缺来源的**默认退出码由 1 改为 0** ⇒ **届时**用 `rc==1` 判断 `params` 成败的脚本，请改用 **`--strict`**（或改成看 `[FAIL]`）。一行迁移示例：

  ```sh
  # 旧：靠默认退出码
  xwl.py params "$P" || exit 1
  # 新：下一版起显式加 --strict 才保留 rc=1
  xwl.py params "$P" --strict || exit 1
  ```
...
- **三条新文档守卫**（`scripts/selftest.py`，**折进既有「文档守卫」聚合行、不新增组号** ⇒ 末尾 `[ok]` 计数不变）：
  - **`17j-12` 跨文件节号指针可解析**：同一格／同一句里同时出现「已知 markdown 文件名」与「节号标记」
    （`§N` / `§N.M` / `§中文` / `第N节`）时，该节号须在目标文件的**标题行**里按**文本**匹配到（**不许按行号**）。
    现状全绿（**纯防回归** —— 判定点的条数随文档增删而变，故不在此写死数字）。
  - **`17j-13` 结构顺序**：① 标题不跳级（与 `17h` 同判据，冗余双保险）；② 带 `## 目录` 的文件，
    TOC 条目锚点须命中正文小节、且**条目顺序与正文出现顺序单调递增**。现状：**跳级 0**、各带目录文件均单调。
  - **`17j-14` 围栏块输出文案符实**（＝把 `17j-6` 从**行内反引号**扩到**代码围栏块**）：围栏内以
    `[note]` / `[warn]` / `[FAIL]` / `[ok]` 开头的行，正文（去空白）**前 16 字**须能在 `xwl.py` 里找到；
    含占位符或正文 <8 字的**跳过**。现状全绿。
  - ⚠️ **`17j-14` 的边界（有意，不是遗漏）**：它用**前缀匹配**，**验不了运行期拼接**出来的文案
    （拼接结果在源码里没有字面前缀）。`examples/README.md` 里正是这种示意输出 ⇒ 该文件**不在扫描面内**
    （扫描面 = `docs` 的 **12 份**）。**纳入会误报。**
- **文档描述去重（只在 `references/` 侧收敛，权威章一字不动）**：
  - **顶层 7 把钥匙的键序字面**：原先在 `SKILL.md` / `references/controls.md` / `references/measured-data.md`
    **各处列一遍** ⇒ 现**只保留 `SKILL.md` §1.3 一处正身**，另 **5 处**改为指针
    （`references/anti-patterns.md` / `checklist.md` / `controls.md` / `measured-data.md` / `sql-fragments.md`）。
  - **注册键 `normalName || itemId` 的定义句**：只剩 `SKILL.md` 第七章一份；`references/` 侧 **3 条定义句**
    改为指针，**引用句 / 口径句 / 行动项一处未删**。定义的正身 = `SKILL.md` §7.1。
  - ⚠️ **本版推翻了仓内一条更早的显式决策**：`[1.3.0]` 段曾记「**判定为有意保留、不改**：顶层键序
    在三处各列一遍…**保留的理由**：…让读者**不必跳文件**」。**反转的理由**：同一串必须逐字精确的键序
    在多个文件各存一份，等于让同一事实有**多个副本** —— 任何一处改动都会让其余副本**静默漂移**，
    而这类漂移在格式层完全合法、没有任何守卫能报出来；代价只是**单文件读者多一次跳转**。
    正身仍逐字精确在 `SKILL.md` §1.3，且与 `xwl.py` 的 `_PAGE_KEYS` 一致。
    因果与作废记录同批写进 `references/workflow-notes.md` §三。
- **口径说明**：三条守卫的现状结果跑 `scripts/selftest.py` 即得；
  本批「描述去重」的处数由**工作区里的一次性报告脚本**得出 —— 它是**施工期工具**、
  **不进本仓分发物**（`git ls-files` 里没有它）。

### 本批补充之三（自检瘦身与提速 · 只动开发期脚本）

> 只改 `scripts/selftest.py`。**`xwl.py` 与它的 15 个子命令一字未动** ⇒ 对外能力不变，版本位不受影响。

- **默认耗时 45.6 s → 24.4 s**：新增 **`--jobs N`**（默认 `min(6, CPU 数)`）把各检查块**并行执行** ——
  每块独占临时目录、`node` 只探测一次并传给子进程、父进程按**固定块序**合并结果。
  `--jobs 1`（43.4 s）与 `--jobs 4`（27.3 s）的 `[ok]` 集合**逐行相同**。
- **新增 `--fast`（8.5 s）**：跳过 `diffguard` 与换行收尾两块，**并显式打印一行 `[note]` 说明跳过了哪两块**
  —— 不静默少跑。**默认档与 CI 仍全跑**，CI 的工作流一字未改。
- **`diffguard` 块的 git 子进程 21 → 7**：`git checkout --` 改为把**基线内容写回文件**；
  `init` + 两次 `config` 改为提交时 `-c` 内联；`add` + `commit` 合并为 `-a`（新文件首入库仍需 `add`）。
  ⚠️ **16 处命令级 `_cli(...)` 调用一处未动** —— 它们测的正是 `rc` / `--strict` / stdout 契约，
  换成进程内调用会改语义，故**保留**（这也是默认档停在 24 s 的原因：剩下的时间几乎全是这 16 次真实进程调用）。
- **删掉一处明文重复**：`17j-13` 的「标题不跳级」臂与 `17h` 同判据（原注释自述"冗余双保险、一处缺陷会报两行"）——
  现只由 `17h` 守；`17j-13` 保留的「目录顺序与正文一致」臂经**注入验证仍有效**（目录乱序 ⇒ 红）。
- **断言一条未删**：末尾 `[ok]` 仍 **116**，与改前**逐条比对差异 0**；`--fast` 为 **114**
  （少的正是被跳过那两块各自的块级 `[ok]`）。
...
> ✅ **（2026-09-21 定案）本版版本号 = `1.3.4`，不再拆分**；`create` 开关（A 组 `A1`–`A4`）**同版随后落地**。
> 位按尺子取**内容的最高级别** ⇒ **`minor`**（含新增对外能力）；**本步（第一步）自身为 patch 性质**（文档结构 + 元纪律）。

### Changed

- **`SKILL.md` 首次结构性下沉（P1+P2）**：把 §一/§二/§三/§四/§五/§七 里「查得到即可、不必占主线」的
  叙述段按**四通道**外移 —— **A** = `references/`（承接细节，主文档留一行指针）、
  **B** = `xwl.py` 的 `help=` / 参数 help / `epilog`、**C** = `test-prompts.json` 的 `expected`、
  **D** = 代码与守卫注释。主文档只留「必用的主线步骤 + 指针」。
  **`SKILL.md` 761 → 660 行**（本批目标终点 ≈658）。
- **篇幅护栏落位**：`SKILL.md` 上限改为 **700 行**、`README.md` 上限改为 **150 行**。
  ⚠️ **上限只是天花板，不是优化目标** —— 原拟的 639 行目标**作废**；上限数字与「落点分流纪律」
  同处写明（`references/workflow-notes.md` §二）。
- **支持矩阵补「操作系统」行**：`SKILL.md` 的适用范围表与 `metadata.json` 的 `limitations`
  各加一行（Windows 与 POSIX 含 macOS 均支持；路径分隔符与换行按各平台惯例，跨平台注意检出 / 存储形态差异）。

### Added

- **两条新守卫**（`scripts/selftest.py`）：① **`--help` 行数上限**（顶层 + 逐子命令，超限即红）；
  ② **支持矩阵 OS 行存在**（`SKILL.md` 适用范围表 + `metadata.json` `limitations` 两处都要有）。
- **`17g`「外移点两侧都在」按四通道补齐 P1+P2 逐点条目**：删主文档指针或删目标文件承载词，任一即红。
- **`patch` 新增 `create` 开关（A 组 `A1`–`A4`，本版第二步）**：`set` 一个**原本不存在**的**对象键**属
  **新建键** ⇒ 该 op 要带 `"create": true`（写成已存在的键上是幂等保护）；`insert`/`append`/`delete` 上带
  `create` 报 **rc=2**；`insert`/`delete` 越界、`delete` 缺失键、以及 `set` 末段数组下标越界，统一改为
  **可读 `[FAIL]` + rc=2**（不再冒裸 `KeyError` / `IndexError`）。
- **中间态（本版不翻默认）**：新建键**当前仍放行**，不带 `create` 时打一行 `[warn]` 预告；
  **默认拒绝 + 新增全局逃生开关**属 **`1.5.0`**（跨版义务已登记到 `references/workflow-notes.md` §四）。
  `--dry-run` 另把带 `create` 的待建键单列一行 `[new-key]`（不带 `create` 的仍走 `[warn]`，两条不混）。
- **两条提示位（`L1` / `K17`，同 `cmd_patch`）**：真跑且**未用 `--backup`** 时补一行 `[warn]`（只加输出、不改 rc）；
  `--dry-run` 对**单行源**改打一行摘要（不再逐行截断 200 字符）。
- **连带生成器自带 `create`**：`recommend_fixes` / `itemids --suggest` / `itemids --name --json` 产出逐条带
  `"create": true`，与新契约同源（避免「生成器与执行器脱钩」）。
- **A 组文档同步**：`SKILL.md` §1.3 / §3.1 各加一例（补顶层钥匙 = 新建键 ⇒ `"create": true`）＋「开关只约束对象键」一句；
  `references/anti-patterns.md` #10、`references/checklist.md` §六 补 `create` 措辞；
  `references/workflow-notes.md` §四登记「翻转默认 + 全局逃生开关」的跨版义务。

### 元纪律（本批新增的「下次别再犯」）

- **落点分流**：只有「必用的主线步骤」才占 `SKILL.md` 的行；细节一律走 A / B / C / D 四通道。
- **每版收尾强报**：`SKILL.md` 行数 + 本版新增落点。
