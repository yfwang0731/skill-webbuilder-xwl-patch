...
>
> 1. **两阶段**：先发"新增开关 + 默认不变 + `[warn]` 预告"，**隔一个已发布版本**再发"翻转默认值"；
...
在 `C:\Work\Project\` 下 **5 个真实 WebBuilder 工程**（合计 25250 个 xwl）上复测，**未改动任何真实文件**：

- **③ 的修复在真实数据上 100% 生效**：`AG-TSHDHY-ILOG-2.0` 是 5 个工程里唯一含 LF 的
