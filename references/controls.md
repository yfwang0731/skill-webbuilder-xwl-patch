...
- [一、怎么读这份表](#一怎么读这份表)
- [二、三套控件库，先选库再选控件](#二三套控件库先选库再选控件)
- [三、控件总表（按设计器面板分组）](#三控件总表按设计器面板分组)
- [四、层级结构：设计器不管，但有三种"约定"](#四层级结构设计器不管但有三种约定)
- [五、"配置载体"节点：最容易看不懂的一类](#五配置载体节点最容易看不懂的一类)
- [六、选型建议（哪种情况用什么）](#六选型建议哪种情况用什么)
- [七、改结构时的安全顺序](#七改结构时的安全顺序)
...
> **口径**：「用过的次数」= 该控件在样本工程里出现的**控件节点数**（一个节点算一次），**不等于文件数**。
> 例：`dataprovider` 是 **1143 个节点**，落在 **1142 个文件**里（有一个文件含 2 个）——两个数字都对，单位不同。
> 本节各表的数字都来自同一份样本统计，**完整口径（样本范围 / 计数方式 / 剔除规则）见
> [`measured-data.md`](measured-data.md)**。两份文件的分工是：**本文件给数字**（读者要用的那一列），
> `measured-data.md` **只给口径**、不重复列数（同一个数字两处维护必然漂移）。

### (根)
| id | ExtJS 类型 | 库 | 容器 | 面板 | 用过的次数（节点数） | 干什么 |
|---|---|---|---|---|---|---|
