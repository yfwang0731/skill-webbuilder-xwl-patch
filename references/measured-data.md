...
**本节的口径与样本范围**（引用本节数字时**必须写明是哪一个**，不可混引）：

- **主口径**（本节数字用的就是这个）—— `new Ext.window.Window(app.X)` 的**出现次数**；判据 =「命中的名字
  ∈ 本文件某个窗口节点的 `itemId` / `normalName`」。**单位**：「**次**」= 一次性数、「**窗**」= 能解析到
  同名窗口节点，故数据形如 **294 次 / 262 窗**。
- **并列口径（全文正则 raw）** —— 对同一批文件做**纯文本正则**扫描（**不按 JSON 树、不剔注释**）另得
  **313 次**：比主口径多 **19** 处 = **5 处注释里的示例代码** + **14 处跨模块或本文件未声明为窗口**
  （`app.<模块>._<窗口>` 这类形态被名字前缀误捕）。两个数**只能按「判据 + 样本范围」互推**，
  **不可当成「两个互相矛盾的数」**。
- **单根样本口径（研究侧）** —— 在**模块根单根样本**上复算：**主口径 90 次 / 84 窗**（**raw 96 次**），
  其中显式 `createInstance:"false"` 的 **2 窗**；与分根表里的「**RO 90**」同源（90 = 主口径的"次"）。
  另一组按**例（次）数**记的值为 **93 例**（其中**非**显式 `createInstance:"false"` 的 **91 例**；
  ⚠️ **91 是「例数」、不是「窗数」**）；`90 < 93 < 96`（**开区间**）⇒ 只能说它**落在主口径 90 与
  raw 96 之间**、**±3 无法唯一归因**（该组按"解析后事件串"计数、更接近主口径 90，其正则细节不可得）。
- **样本与范围** —— 样本 = 三个 wb 根的 `modules/**`（排除 `libs/`）；本节窗口数按**可解析**口径
  （单根样本 **2833 个 xwl**、**2775** 可解析）；与 **§十 / §11.3** 的计数**不是同一口径**。

- **`new Ext.window.Window(app.X)`**（无下划线当构造参数）：**294 次 / 262 窗**
  （分根 RO 90 / ESCMS 24 / TSHDHY 180）；其中 `createInstance` 缺省（=真）**248** / `"false"` **14**；
  `closeAction` 缺省 **91 窗** / `hide` **137 窗** / `destroy` **34 窗**。
  ⚠️ 严重度**按路线分**：`destroy` 路线 **34 窗 = 确定缺陷**（第二次打开为空白窗）；
  `hide` / 缺省 **228 窗 = 能跑但有副作用**（重复 id、实例泄漏）。
- **回写写法** `app.X = new Ext.window.Window(app.X)`：**仅 3 处**（三个工程同源，均落在同一处上传弹窗）。
- **加载时 `new` 窗口**：**1222** 处（其中在 `module` 上 **1132** = `finalize` 1094 + `initialize` 38）。
- **窗口父节点分布**：`module` **1137** / `viewport` **1** / `picker` **1**。
- **常驻（`hide`）窗口内 grid/store 的 `autoLoad:true`**：**722（43%）** ⇒ 与「建议不要 autoLoad」相反，属普遍偏离。
- **窗口内首个子控件**：`panel`/`form` **3961**；`grid` **431**（其中 `destroy` 285 = 66%）。
- **写法与 `closeAction` 配套**：教科书式常驻 **847** / 教科书式重建 **1772**；混搭普遍（`destroy` 窗里 **223** 个无任何打开调用）。
